...
public interface UserJangterController {
    ...
    CommonResponse<PageImpl<UserPurchaseResponseDTO>> findUserPurchaseList(PrincipalUser principalUser, Long userId , Pageable pageable);

    @Operation(summary = "유저 장터 상품 판매 목록", description = "유저가 장터에서 사용자가 판매한 물품의 목록들을 보여줍니다.")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "API 요청 성공"),
        @ApiResponse(responseCode = "400", description = "입력한 값의 유효성이 올바르지 않을 때"),
        @ApiResponse(responseCode = "403", description = "사용자 로그인이 되어있지 않았을 때")
    ...
    CommonResponse<PageImpl<UserCellResponseDTO>> findUserCellList(PrincipalUser principalUser, Long userId , Pageable pageable);
