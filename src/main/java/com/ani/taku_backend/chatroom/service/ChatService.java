...
public class ChatService {
    ...
    public void sendMessageByWsRoomId(String wsRoomId, Long senderId, String content) {
        ...
        ChatRoomMetaInfo chatRoomMetaInfo = chatroomMetaRepository.findByChatRoomId(chatRoom.getId())
                .orElseThrow(() -> new DuckwhoException(ErrorCode.CHAT_ROOM_NOT_FOUND));
        log.info("채팅방 메타 정보 조회 완료 - participants: {}", chatRoomMetaInfo.getParticipants().getInfo());

        // 상대방의 메시지 스톡 증가
        chatRoomMetaInfo.getParticipants().getInfo().forEach((userId, participantInfo) -> {
            log.info("참가자 정보 처리 - userId: {}, role: {}, currentStock: {}, isConnected: {}", 
            ...
            if (!userId.equals(senderId)) {
                ...
                log.info("메시지 스톡 증가 - userId: {}, newStock: {}", userId, participantInfo.getMessageStock());
            }
        });
    ...
    public void markMessagesAsReadByWsRoomId(String wsRoomId, Long userId) {
        log.info("메시지 읽음 처리 시작 - wsRoomId: {}, userId: {}", wsRoomId, userId);
        ...
        log.info("전체 메시지 수: {}", messages.size());
        
        // 메시지 상태 로깅
        messages.forEach(message -> {
            log.info("메시지 상태 - messageId: {}, senderId: {}, content: {}, read: {}, sentAt: {}", 
        ...
        for (ChatMessage message : messages) {
            if (!message.getSenderId().equals(userId)) {
                unreadCount++;
                log.info("안 읽은 메시지 발견 - messageId: {}, senderId: {}, content: {}, sentAt: {}", 
                    message.getId(), message.getSenderId(), message.getContent(), message.getSentAt());
                message.setRead(true);
                chatMessageRepository.save(message);
            }
        }
        log.info("안 읽은 메시지 총 개수: {}", unreadCount);
        
        // 메시지 스톡도 초기화
        ChatRoomMetaInfo chatRoomMetaInfo = chatroomMetaRepository.findByChatRoomId(chatRoom.getId())
                .orElseThrow(() -> new DuckwhoException(ErrorCode.CHAT_ROOM_NOT_FOUND));
        
        log.info("채팅방 참가자 정보: {}", chatRoomMetaInfo.getParticipants().getInfo());
        
        ParticipantInfo participantInfo = chatRoomMetaInfo.getParticipants().getInfo().get(userId);
        if (participantInfo != null) {
            log.info("메시지 스톡 초기화 전: {}", participantInfo.getMessageStock());
            participantInfo.resetMessageStock();
            log.info("메시지 스톡 초기화 후: {}", participantInfo.getMessageStock());
            chatroomMetaRepository.save(chatRoomMetaInfo);
        }
    }
